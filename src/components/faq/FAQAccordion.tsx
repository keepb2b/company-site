    ...
    <div className="space-y-3">
      {items.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className={`scroll-reveal overflow-hidden rounded-2xl border bg-white/95 shadow-[0_14px_38px_rgba(30,51,71,0.05)] ring-1 ring-white/70 backdrop-blur transition-all duration-300 ${
              open
                ? 'border-aizome-300 shadow-[0_22px_58px_rgba(30,51,71,0.11)]'
                : 'border-sand-200 hover:border-aizome-200 hover:shadow-[0_18px_48px_rgba(30,51,71,0.08)]'
            }`}
          >
            <button
              type="button"
              ...
              <span className="flex min-w-0 items-start gap-4">
                <span
                  className={`mt-0.5 shrink-0 rounded-full px-3 py-1 font-display text-xs font-bold transition-colors ${
                    open ? 'bg-coral-500 text-white' : 'bg-coral-500/10 text-coral-600'
                  }`}
                >
                  Q.{item.id}
                </span>
