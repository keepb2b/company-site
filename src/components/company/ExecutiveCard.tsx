...
export function ExecutiveCard({ role, roleEn, name, message }: Executive) {
  return (
    <article className="executive-card group scroll-reveal overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
      <div className="border-b border-aizome-300/45 bg-gradient-to-br from-sand-50 to-aizome-300/30 px-5 py-6 md:px-6 md:py-7">
        <p className="font-display text-xs font-semibold uppercase tracking-[0.2em] text-navy-700/70">{roleEn}</p>
        <h3 className="mt-2 text-2xl font-bold text-navy-950 md:text-[1.75rem]">{name}</h3>
        <p className="mt-1 text-sm text-navy-700/80">{role}</p>
      </div>
